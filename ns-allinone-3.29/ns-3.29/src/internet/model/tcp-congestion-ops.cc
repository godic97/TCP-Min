  ...
  if (segmentsAcked >= 1)
    {
      ...
      double tmp = static_cast<double> (tcb->m_cWnd);
      tcb->m_cWnd = std::max(tmp, 50.0 * tcb->m_segmentSize);
      NS_LOG_INFO ("In SlowStart, updated to cwnd " << tcb->m_cWnd << " ssthresh " << tcb->m_ssThresh);
      std::cout << "<Slow Start> cWnd: " << tcb->GetCwndInSegments() << " ssthresh: " << tcb->GetSsThreshInSegments() << " lastRTT: " << tcb->m_lastRtt <<" last Acked seqs: " << tcb->m_lastAckedSeq << " CA State: " << tcb->m_congState << " this is: " << this <<  '\n'; 
...
TcpNewReno::CongestionAvoidance (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
 ...
 //     double adder = static_cast<double> (tcb->m_cWnd.Get()/2);
...
//      std::cout << "<Congesion Avoidance> cWnd: " << tcb->GetCwndInSegments() << " ssthresh: " << tcb->GetSsThreshInSegments()  <<" lastRTT: " << tcb->m_lastRtt  <<" last Acked seqs: " << tcb->m_lastAckedSeq << " CA State: " << tcb->m_congState << " this is:" << this << '\n';
